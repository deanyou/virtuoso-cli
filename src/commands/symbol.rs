...
pub fn inspect(
    ...
    // Unchecked — the SKILL is generated by SymbolOps from typed arguments, not
    // supplied by the caller, and the capability check already ran at RPC
    // dispatch. Re-checking here locks non-Admin users out of their own typed
    // RPC (`symbol.inspect`), which is the very thing the gate is not for.
...
pub fn generate(
    ...
    let client = VirtuosoClient::from_context(ctx)?;
    // Unchecked — see `inspect` above. Without this, `symbol.generate` is
    // unreachable for non-Admin callers, which makes hierarchical design
    // (schematic -> symbol -> instantiate in a testbench) impossible over
    // typed RPC.
    let r = client.execute_skill_unchecked(
