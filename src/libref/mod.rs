...
mod tests {
    ...
    fn corpus() -> LibRefFinder {
        LibRefFinder::for_test(vec![
            ...
            LibSymbol {
                ...
                params: vec![
                    param("ia", "Amplitude 1 (Apk)", ""),
                    param("idc", "DC current", ""),
                ],
                release: "IC231".into(),
                source_file: "independent.html".into(),
                ..Default::default()
    ...
    fn label_matches_outrank_description_only_matches() {
        ...
        symbols.push(LibSymbol {
            ...
            params: vec![param(
                "zz",
                "Modulation",
                "Sets the amplitude modulation type",
            )],
            ..Default::default()
        });
    ...
    fn the_guessed_name_matches_nothing() {
        assert!(corpus()
            .search_params("ampl", SearchMode::Exact, 10)
            .is_empty());
    }
    ...
    fn fuzzy_symbol_search_also_matches_title_and_category() {
        let f = corpus();
        assert_eq!(
            f.search("sinusoidal", SearchMode::Fuzzy, 10)[0].name,
            "vsin"
        );
    ...
    fn probe_output_is_deduplicated_in_discovery_order() {
        ...
        assert_eq!(
            ...
            vec![
                (
                    "analogLib",
                    "/opt/Cadence/IC231/doc/analoglibref".to_string()
                ),
    ...
    fn release_tag_strips_any_registered_manual_not_just_analoglib() {
        assert_eq!(
            release_tag("/opt/Cadence/IC231/doc/basicLib"),
            Some("IC231")
        );
        assert_eq!(
            release_tag("/opt/Cadence/IC231/doc/rflibrary"),
            Some("IC231")
        );
        assert_eq!(
    ...
    fn only_documentation_files_count_as_cache_content() {
        ...
        assert!(is_doc_file(std::path::Path::new(
            "IC231__analoglibref.json"
        )));
    ...
    fn a_release_root_is_a_valid_load_target_too() {
        ...
        f.load(root.path()).unwrap();
        assert_eq!(
            f.get_in("basic", "opin").unwrap().description,
            "An output pin."
        );
    }
