...
fn parse_release(dir: &Path, prefix: &str) -> (Vec<LibSymbol>, ParseReport) {
    ...
    let appendix = parse_appendix(&read_lossy(
        &dir.join(format!("{prefix}{APPENDIX_BASENAME}")),
    ));
...
mod tests {
    ...
    fn anchors_are_found_even_when_split_across_the_heading() {
        ...
        assert!(
            html.find("Symbol: bsource").is_none(),
            "the naive scan fails"
        );
    ...
    fn malformed_or_unbounded_ranges_expand_to_nothing() {
        ...
        assert!(
            expand_range("F1 - F1").is_empty(),
            "a range of one is a name"
        );
        assert!(
    ...
    fn appendix_disambiguates_a_reused_cdf_name_by_label() {
        ...
        assert!(
            !a.contains_key("CDF Parameter Name"),
    ...
    fn value_tables_are_not_mistaken_for_parameter_tables() {
        ...
        assert_eq!(
            skipped, 2,
            "the two value rows are counted, not silently lost"
        );
    }
    ...
    fn cross_reference_stub_never_shadows_the_full_entry() {
        for order in [
            vec![
                sym("port", "independent.html", 48),
                sym("port", "ports.html", 0),
            ],
            vec![
                sym("port", "ports.html", 0),
                sym("port", "independent.html", 48),
