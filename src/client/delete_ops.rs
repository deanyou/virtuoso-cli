...
use crate::client::schematic_ops::{cv_guard, term_point, EDIT_CV};
...
/// Which wire figure `schematic.delete_figure` is aimed at.
///
/// A figure has no name — instances do, properties do, cellviews do, but a wire
/// is just geometry — so it has to be pointed at. Two ways, because there are
/// two ways a stray wire is encountered:
///
/// * [`Self::Term`] — *"the stub on `M1/G`"*. This is the one
///   `schematic.label_term` asks for by name when it refuses to relabel a
///   terminal that already carries somebody else's stub.
/// * [`Self::Point`] — *"whatever wire is at (2.6 3)"*, for a wire with no
///   label to name it by and no terminal to hang it off.
#[derive(Debug, Clone, PartialEq)]
pub enum FigureTarget {
    Term { inst: String, term: String },
    Point { x: f64, y: f64 },
}

impl FigureTarget {
    pub fn term(inst: &str, term: &str) -> Result<Self> {
        let (inst, term) = (inst.trim(), term.trim());
        if inst.is_empty() || term.is_empty() {
            return Err(VirtuosoError::Config(
                "schematic.delete_figure: 'inst' and 'term' must both be non-empty".into(),
            ));
        }
        Ok(Self::Term {
            inst: inst.to_string(),
    ...
    pub fn point(x: f64, y: f64) -> Result<Self> {
        if !x.is_finite() || !y.is_finite() {
            return Err(VirtuosoError::Config(
                "schematic.delete_figure: 'x' and 'y' must be finite numbers".into(),
            ));
        }
        Ok(Self::Point { x, y })
    }

    /// `M1/G` or `(2.6 3)` — what the manifest calls this target.
    pub fn describe(&self) -> String {
        match self {
            Self::Term { inst, term } => format!("{inst}/{term}"),
            Self::Point { x, y } => format!("({x} {y})"),
        }
    }

    /// SKILL that binds `pt` to the point this target names.
    ///
    /// The terminal form defers to [`term_point`], the same locator
    /// `schematic.label_term` uses to decide where to *draw* a stub. Two
    /// different computations of "where is `M1/G`" would make a stub drawable
    /// but not deletable.
    fn locate(&self, action: &str) -> String {
        match self {
            Self::Term { inst, term } => term_point(action, inst, term),
            Self::Point { x, y } => format!("pt = list({x:?} {y:?})"),
        }
    }
}

/// SKILL generators for the five destructive operations and their read-only
...
impl DeleteOps {
    ...
    /// Read-only: the wire figure `delete_figure` would remove, and everything
    /// that would go with it.
    ///
    /// `figures` is read out of the database, not out of what the caller named:
    /// a wire label is a **child** of its wire (measured on
    /// `SCRATCH_LIB/_figmove_probe`, 2026-09-11 — `label~>parent` *is* the
    /// `line`), so `schDelete` on the wire takes the label too, and the manifest
    /// can promise a number instead of hedging.
    ///
    /// Check markers are the one thing `schDelete` takes that the manifest does
    /// **not** count. `schematic.check` hangs a `marker` child on every wire it
    /// has something to say about (measured 2026-09-12: `kids` went 1 → 2 on all
    /// four stubs after one `check`), and they are regenerated by the next
    /// check. Counting them would make `figures` jump between 2 and 3 for the
    /// same wire — and since the confirmation token is bound to the manifest,
    /// running `check` while a human read the manifest would silently void
    /// their approval.
    pub fn probe_figure(&self, target: &FigureTarget) -> String {
        let locate = target.locate("schematic.delete_figure");
        let manifest = figure_manifest(target);
        let guard = cv_guard();
        let lib_guard = lib_guard("schematic.delete_figure");
        format!(
            r#"let((cv inst mterm pin fig bb pc pt wire txt sep n ptxt) cv = {EDIT_CV} {guard} {lib_guard} {locate} {manifest} sprintf(nil "{{\"action\":\"schematic.delete_figure\",\"lib\":%L,\"cell\":%L,\"view\":%L,\"target\":%L,\"point\":%L,\"bBox\":%L,\"labels\":[%s],\"figures\":%d}}" cv~>libName cv~>cellName cv~>viewName "{}" ptxt sprintf(nil "%L" wire~>bBox) txt n))"#,
            escape_skill_string(&target.describe())
        )
    }

    /// `schDelete(d_fig)` on a wire — the counterpart to `delete_instance`, and
    /// the way out of the two dead ends `schematic.label_term` reports: *"…
    /// already carries a stub labelled X"* and *"… a wire already meets X/Y but
    /// carries no label"*. Both used to name a delete that did not exist.
    ///
    /// One call is enough. The label rides along as a child, so there is no
    /// second `schDelete` here and no window in which the wire is gone but its
    /// label is still floating over an empty patch of schematic.
    ///
    /// In memory only, like every edit in this file — `saved: false`.
    pub fn delete_figure(&self, target: &FigureTarget) -> String {
        let locate = target.locate("schematic.delete_figure");
        let manifest = figure_manifest(target);
        let guard = cv_guard();
        let lib_guard = lib_guard("schematic.delete_figure");
        let what = escape_skill_string(&target.describe());
        format!(
            r#"let((cv inst mterm pin fig bb pc pt wire txt sep n ptxt ok) cv = {EDIT_CV} {guard} {lib_guard} {locate} {manifest} ok = schDelete(wire) unless(ok error("%s" "schematic.delete_figure: schDelete refused the wire at {what}")) sprintf(nil "{{\"status\":\"ok\",\"action\":\"schematic.delete_figure\",\"lib\":%L,\"cell\":%L,\"view\":%L,\"target\":%L,\"point\":%L,\"labels\":[%s],\"figures\":%d,\"saved\":false}}" cv~>libName cv~>cellName cv~>viewName "{what}" ptxt txt n))"#
        )
    }
...
/// SKILL that finds the wire at `pt` and describes what deleting it costs.
///
/// Shared verbatim between [`DeleteOps::probe_figure`] and
/// [`DeleteOps::delete_figure`] so the manifest the caller confirms is computed
/// by the same code that then does the deleting — a probe that locates the wire
/// one way and an execute that locates it another way is a two-phase confirm
/// that confirms the wrong thing.
///
/// Assumes `cv` and `pt` are bound, and binds `wire`, `ptxt`, `n` (the design
/// figures that will die — the wire and its labels, not the disposable check
/// markers), `txt` (the labels, as JSON array elements) and `sep`.
fn figure_manifest(target: &FigureTarget) -> String {
    let what = escape_skill_string(&target.describe());
    format!(
        r#"ptxt = sprintf(nil "(%g %g)" car(pt) cadr(pt)) wire = car(setof(f dbGetOverlaps(cv list(pt pt)) f~>objType == "line")) when(!wire error("%s" sprintf(nil "schematic.delete_figure: no wire figure at {what} %s — nothing to delete there" ptxt))) n = 1 txt = "" sep = "" foreach(k wire~>children when(k~>objType == "label" n = n+1 txt = strcat(txt sep sprintf(nil "%L" k~>theLabel)) sep = ","))"#
    )
}
...
mod tests {
    ...
    fn dd_get_obj_always_passes_an_explicit_read_mode_in_the_sixth_slot() {
        ...
        assert_eq!(
            cell.dd_args(),
            r#""DESIGN_LIB" "_rbscratch" nil nil nil "r""#
        );
    ...
    fn schematic_edits_check_the_allow_set_where_the_library_name_is_known() {
        ...
        for s in [
            ...
            ops.probe_figure(&FigureTarget::term("M1", "G").unwrap()),
            ops.delete_figure(&FigureTarget::term("M1", "G").unwrap()),
            ops.probe_figure(&FigureTarget::point(2.6, 3.0).unwrap()),
            ops.delete_figure(&FigureTarget::point(2.6, 3.0).unwrap()),
        ] {
    ...
    fn deletes_use_the_documented_api_for_each_layer() {
        ...
        assert!(
            prop.contains(r#"dbSearchPropByName(inst "ampl")"#),
            "{prop}"
        );
        assert!(
            prop.contains(r#"dbDeletePropByName(inst "ampl")"#),
            "{prop}"
        );
    }
    ...
    fn the_interactive_delete_is_never_used() {
        ...
        let fig = FigureTarget::term("M1", "G").expect("valid");
        assert!(!DeleteOps.delete_figure(&fig).contains("schHiDelete"));
        // Same reasoning for the move side: `schHiMove` is what the M key runs,
        // and it prompts for points.
        assert!(!DeleteOps.delete_figure(&fig).contains("schHiMove"));
    }
    ...
    fn in_memory_edits_report_that_they_are_unsaved() {
        ...
        assert!(DeleteOps
            .delete_instance("M1")
            .contains(r#"\"saved\":false"#));
        assert!(DeleteOps
            .delete_prop("VIN", "ampl")
            .contains(r#"\"saved\":false"#));
        let fig = FigureTarget::term("M1", "G").expect("valid");
        assert!(DeleteOps.delete_figure(&fig).contains(r#"\"saved\":false"#));
    }
    ...
    fn schematic_edits_go_through_the_cellview_guard() {
        ...
        for s in [
            ...
            DeleteOps.delete_figure(&FigureTarget::term("M1", "G").unwrap()),
            DeleteOps.delete_figure(&FigureTarget::point(2.6, 3.0).unwrap()),
        ] {
    ...
    fn by_term() -> FigureTarget {
        FigureTarget::term("M1", "G").expect("valid")
    }

    fn by_point() -> FigureTarget {
        FigureTarget::point(2.6, 3.0).expect("valid")
    }

    /// A figure is addressed one way or the other, never half a way.
    #[test]
    #[serial]
    fn a_figure_target_needs_both_halves_of_its_pair() {
        let _env = configured();
        for (inst, term) in [("", "G"), ("M1", ""), ("   ", "  ")] {
            assert!(FigureTarget::term(inst, term).is_err(), "{inst:?}/{term:?}");
        }
        for (x, y) in [(f64::NAN, 0.0), (0.0, f64::INFINITY)] {
            assert!(FigureTarget::point(x, y).is_err(), "({x} {y})");
        }
    }

    #[test]
    #[serial]
    fn a_figure_target_describes_itself_for_the_manifest() {
        let _env = configured();
        assert_eq!(by_term().describe(), "M1/G");
        assert_eq!(by_point().describe(), "(2.6 3)");
    }

    /// The terminal form must resolve the point exactly the way
    /// `schematic.label_term` does when it *draws* a stub there. Two different
    /// answers to "where is M1/G" would make a stub drawable but not deletable.
    #[test]
    #[serial]
    fn the_terminal_form_reuses_the_label_term_locator() {
        let _env = configured();
        let locator = term_point("schematic.delete_figure", "M1", "G");
        for s in [
            DeleteOps.probe_figure(&by_term()),
            DeleteOps.delete_figure(&by_term()),
        ] {
            assert!(s.contains(&locator), "locator not shared: {s}");
            assert!(s.contains("dbTransformPoint(pc inst~>transform)"), "{s}");
        }
    }

    /// The point form takes the caller at their word — no instance lookup, no
    /// transform, just the coordinates.
    #[test]
    #[serial]
    fn the_point_form_binds_the_coordinates_directly() {
        let _env = configured();
        for s in [
            DeleteOps.probe_figure(&by_point()),
            DeleteOps.delete_figure(&by_point()),
        ] {
            assert!(s.contains("pt = list(2.6 3.0)"), "{s}");
            assert!(!s.contains("dbTransformPoint"), "{s}");
            assert!(!s.contains("~>master~>terminals"), "{s}");
        }
    }

    /// Only wires. `dbGetOverlaps` at a terminal returns the instance and its
    /// pin labels too, and `schDelete` would happily take any of them.
    #[test]
    #[serial]
    fn only_a_line_is_ever_selected_for_deletion() {
        let _env = configured();
        for t in [by_term(), by_point()] {
            for s in [DeleteOps.probe_figure(&t), DeleteOps.delete_figure(&t)] {
                assert!(s.contains(r#"f~>objType == "line""#), "{s}");
            }
        }
    }

    /// One `schDelete`, on the wire. The label is a child and rides along —
    /// measured on `SCRATCH_LIB/_figmove_probe`, 2026-09-11 — so a second
    /// delete would be aimed at an object that no longer exists.
    #[test]
    #[serial]
    fn deleting_a_figure_is_one_schdelete_on_the_wire() {
        let _env = configured();
        for t in [by_term(), by_point()] {
            let s = DeleteOps.delete_figure(&t);
            assert_eq!(s.matches("schDelete(").count(), 1, "{s}");
            assert!(s.contains("schDelete(wire)"), "{s}");
        }
    }

    /// The manifest counts the design figures that will die — the wire and its
    /// labels — and deliberately leaves out the `marker` children that
    /// `schematic.check` hangs on a wire, so running a check does not change
    /// the number and void a token that was already minted.
    #[test]
    #[serial]
    fn the_manifest_counts_the_labels_but_not_the_check_markers() {
        let _env = configured();
        for t in [by_term(), by_point()] {
            for s in [DeleteOps.probe_figure(&t), DeleteOps.delete_figure(&t)] {
                assert!(
                    !s.contains("length(wire~>children)"),
                    "markers counted: {s}"
                );
                assert!(
                    s.contains(r#"foreach(k wire~>children when(k~>objType == "label" n = n+1"#),
                    "{s}"
                );
                assert!(s.contains("k~>theLabel"), "{s}");
            }
        }
    }

    /// What phase 1 shows and what phase 2 deletes are found by the same code.
    /// A probe that locates the wire one way and an execute that locates it
    /// another way is a confirmation of the wrong thing.
    #[test]
    #[serial]
    fn probe_and_execute_locate_the_wire_identically() {
        let _env = configured();
        for t in [by_term(), by_point()] {
            let shared = figure_manifest(&t);
            assert!(DeleteOps.probe_figure(&t).contains(&shared));
            assert!(DeleteOps.delete_figure(&t).contains(&shared));
        }
    }

    #[test]
    #[serial]
    fn figure_names_are_escaped_before_they_reach_skill() {
        let _env = configured();
        let t = FigureTarget::term("bad\"inst", "G").expect("valid");
        for s in [DeleteOps.probe_figure(&t), DeleteOps.delete_figure(&t)] {
            assert!(!s.contains(r#""bad"inst""#), "unescaped quote: {s}");
            assert!(s.contains(r#"bad\"inst"#), "{s}");
        }
    }
}
