...
fn reject_undeclared_params(method: &str, params: &Value) -> Result<()> {
    let (Some((summary, declared)), Some(obj)) = (DECLARED_PARAMS.get(method), params.as_object())
    else {
        return Ok(());
    };
...
fn json_coord_or(value: Option<&Value>, field: &str, default: f64) -> Result<f64> {
    match value {
        ...
        Some(v) => v.as_f64().filter(|f| f.is_finite()).ok_or_else(|| {
            VirtuosoError::Execution(format!(
...
fn parse_wire_points(value: Option<&Value>) -> Result<Vec<(f64, f64)>> {
    ...
    let bad = |i: usize, what: &str| VirtuosoError::Execution(format!("points[{i}]: {what}"));
    arr.iter()
        .enumerate()
        .map(|(i, v)| {
            let s = v
                .as_str()
                .ok_or_else(|| bad(i, "expected a \"x,y\" string"))?;
            let (x, y) = s
                .split_once(',')
...
mod tests {
    ...
    fn an_undeclared_param_is_refused_not_silently_dropped() {
        ...
        let err = reject_undeclared_params(
            "schematic.place",
            &json!({"master": "analogLib/res", "name": "R1", "z": 3}),
        )
        .unwrap_err()
        .to_string();
        ...
        assert!(
            !err.contains("'master'"),
            "declared params must not be listed: {err}"
        );
    }
    ...
    fn every_param_a_dispatch_arm_reads_is_declared_in_the_schema() {
        ...
        assert!(
            checked > 100,
            "only {checked} param reads found — parser is broken"
        );
    }
